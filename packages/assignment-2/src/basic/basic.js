...
export function deepEquals(target1, target2) {
  ...
  if (typeof target1 === 'object') {
    ...
    if (Array.isArray(target1) && Array.isArray(target2)) {
      ...
      //2.4. 객체인지 체크 후 키의 길이가 같은지 체크 후 각 키의 값을 뽑고 각 키의 값이 같은지 돌면서 깊은비교
      //case. { a: 1 }==={ a: 1 },{ a: 1 }==={ a: 2 },{ a: 1, b: { c: 2, d: [], e: [1, 2, 3] } }==={ a: 1, b: { c: 2, d: [], e: [1, 2, 3] } },{ a: 1, b: { c: 2, d: [], e: [1, 2, 3], f: new Number(1) } }==={ a: 1, b: { c: 2, d: [], e: [1, 2, 3], f: new Number(1) } }
      const keys1 = Object.keys(target1);
      const keys2 = Object.keys(target2);
      if (keys1.length !== keys2.length) return false;
...
// 얘는 언제쓰지? 왜 쓰는거지? : 값을 숨기고 싶을때 주로 테스트 코드 사용시 -> 의도적으로 값을 변환하거나 예측대로 흘러가게 만든다거나 할때 사용, 실제 서비스에는 사용하지않지만 복잡한 데이터에서는 사용할 수 있다. 숨기고 싶은 로직이 도출될수 있다. 인스턴스 속성을 쓰면 직접적으로 접근할 수 없다. 그래서 getter, setter를 사용하여 접근할 수 있게 만들어준다. 은닉화 하는것이다.
